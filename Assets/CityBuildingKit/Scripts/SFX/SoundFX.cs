...
{       //attached to the camera, generates sounds
    ...
    IEnumerator StartSounds()
    {
        while (true)
        {
            ...
            soundSource.Play();
            yield return new WaitForSeconds(sound4.length);
            soundSource.clip = sound5;
            soundSource.Play();
            yield return new WaitForSeconds(sound5.length);
            yield return null;
        }
    }
